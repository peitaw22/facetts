...
public:
  ...
  std::vector<Node*> Search_leafnode(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS]) const;
  int Search_circle(ELEMTYPE &range ,const ELEMTYPE a_min[NUMDIMS],  std::function<bool (const DATATYPE&)> callback) const;
  void Search_cir_leaf(ELEMTYPE &range, const ELEMTYPE a_min[NUMDIMS], std::vector<Node * > &vec_l) const;
...
public://変更
  ...
  struct Node
  {
    ...
    std::unordered_map<int, Branch > mp;           
  };
  ...
  bool Overlap_circle(ELEMTYPE &range, Rect* a_rectA, Rect* a_rectB) const;

  void ReInsert(Node* a_node, ListNode** a_listNode);
  bool Search(Node* a_node, Rect* a_rect, int& a_foundCount, std::function<bool (const DATATYPE&)> callback) const;

  bool Search_leafnode(Node* a_node, Rect* a_rect, std::vector<Node *> &vec_l ) const;
  bool Search_circle (ELEMTYPE& range , Node* a_node, Rect* a_rect, int& a_foundCount, std::function<bool (const DATATYPE&)> callback) const;
  bool Search_cir_leaf (ELEMTYPE& range , Node* a_node, Rect* a_rect, std::vector<Node * > &vec_l) const;
  void MakeHash();
  void Make (Node* a_node);
...
RTREE_TEMPLATE
std::vector< typename RTREE_QUAL :: Node *> RTREE_QUAL::Search_leafnode(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS]) const
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
  ...
  std::vector<Node *> vec_l;
  Search_leafnode(m_root, &rect, vec_l);
...
RTREE_TEMPLATE
int RTREE_QUAL::Search_circle(ELEMTYPE &range ,const ELEMTYPE a_min[NUMDIMS], std::function<bool (const DATATYPE&)> callback) const
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
  ...
  Rect rect; //ただ円の座標を入れるだけのもの

  for(int axis=0; axis<NUMDIMS; ++axis)
  {
    rect.m_min[axis] = a_min[axis];
    rect.m_max[axis] = a_min[axis];
  }

  // NOTE: May want to return search result another way, perhaps returning the number of found elements here.

  int foundCount = 0;
  Search_circle(range ,m_root, &rect, foundCount, callback);

  return foundCount;
}

RTREE_TEMPLATE
void RTREE_QUAL::Search_cir_leaf(ELEMTYPE &range ,const ELEMTYPE a_min[NUMDIMS], std::vector<Node * > &vec_l) const
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
  ...
  Search_cir_leaf(range, m_root, &rect, vec_l);
...
RTREE_TEMPLATE
bool RTREE_QUAL::Overlap(Rect* a_rectA, Rect* a_rectB) const//Aが探索範囲
{
  ASSERT(a_rectA && a_rectB);
...
RTREE_TEMPLATE
bool RTREE_QUAL::Overlap_circle(ELEMTYPE &range, Rect* a_rectA, Rect* a_rectB) const
{
  ASSERT(a_rectA && a_rectB);
  ELEMTYPE tmp=0; //円と矩形の距離を格納

  //円の各座標はa_rectA->m_min[index]で表す
  for(int index=0; index < NUMDIMS; ++index){
      if(a_rectA->m_min[index] < a_rectB->m_min[index]){
          tmp += (a_rectA->m_min[index] - a_rectB->m_min[index]) * (a_rectA->m_min[index] - a_rectB->m_min[index]);
      }
      if(a_rectA->m_min[index] > a_rectB->m_max[index]){
          tmp += (a_rectA->m_min[index] - a_rectB->m_max[index]) * (a_rectA->m_min[index] - a_rectB->m_max[index]);
      }
  }
  if(range < sqrt(tmp)){
      return false;
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search_leafnode(Node* a_node, Rect* a_rect,  std::vector< Node *>  &vec_l) const
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
    ...
    if(a_node->IsInternalNode()){
        ...
        for(int index=0; index < a_node->m_count; ++index){
            if(Overlap(a_rect, &a_node->m_branch[index].m_rect)){
                ...
                if(!Search_leafnode(a_node->m_branch[index].m_child, a_rect, vec_l )){
                    // The callback indicated to stop searching
                    return false;
                }               
            }
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search_circle(ELEMTYPE &range ,Node* a_node, Rect* a_rect, int& a_foundCount, std::function<bool (const DATATYPE&)> callback) const
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  ...
  if(a_node->IsInternalNode())
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap_circle(range, a_rect, &a_node->m_branch[index].m_rect))
      {
        if(!Search_circle(range, a_node->m_branch[index].m_child, a_rect, a_foundCount, callback))
        {
          // The callback indicated to stop searching
          return false;
        }
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search_cir_leaf(ELEMTYPE &range, Node* a_node, Rect* a_rect,  std::vector< Node *>  &vec_l) const
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  ...
  if(a_node->IsInternalNode())
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap_circle(range, a_rect, &a_node->m_branch[index].m_rect))
      {
        if(!Search_cir_leaf(range, a_node->m_branch[index].m_child, a_rect, vec_l))
        {
          // The callback indicated to stop searching
          return false;
        }
...
RTREE_TEMPLATE
void RTREE_QUAL::MakeHash(){
  Make(m_root);
}

RTREE_TEMPLATE
void RTREE_QUAL::Make(Node* a_node){
  //Make(m_root);
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ...
  if(a_node->IsInternalNode()){
    ...
    for(int index =0; index < a_node->m_count; ++index){
      Make(a_node->m_branch[index].m_child);
    }

  }else{
    // This is a leaf node
    for(int i=0; i < a_node->m_count; i++){
      a_node->mp[i] = a_node->m_branch[i];
    }
  }
